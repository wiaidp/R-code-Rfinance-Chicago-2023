...
LPD <- function(cache, params, list_layer_size,linear_output,atan_not_sigmoid)
{
  ...
  if (n_y>1)
  {
    print("Current code works for a single output neuron only")
    return()
  }
...
# Runs backwards i.e. ordering of n_h is reversed
      ...
      } else
      {
        ...
        if (F)
        {
# Alternative expression: consistent with paper since we exchanged ordering of dA_list[[i-1]] and params$W_list[[length(n_h)+2-i]]%*%t(dA_list_forward[[length(n_h)+1-i]][,k,])
          ddA_list[[i]]<-(t(params$W_list[[length(n_h)+3-i]])%*% ddA_list[[i-1]])*da+
            (t(params$W_list[[length(n_h)+3-i]])*da*(1-2*cache$A_list[[length(n_h)+2-i]])*
             params$W_list[[length(n_h)+2-i]]%*%t(dA_list_forward[[length(n_h)+1-i]][,k,])%*%dA_list[[i-1]])
        }
      }
    }
  } else
...
# Complete Hessian at time point t: this code verifies formula in XAI-paper
# Tough call because it mixes backpropagation (in dA_list) with forward propagation (dA_list_forward)
#   In the derivative of the complex gradient product one needs at each layer the gradient from input to hidden (forward prop)
#   But the Hessian is contructed recursively from output to input (backpropagation)
Hessian_t <- function(dA_list,cache, params, list_layer_size,linear_output,atan_not_sigmoid,dA_list_forward,t)
{
  # params<-parm  t<-1
  m <- dim(cache$A_list[[length(cache$A_list)]])[2]
  ...
  } else
  {
    ...
    {
      # Derivative of atan(x) is 1/(1+x^2): double derivative is -2x/(1+x^2)^2
      ddA_list[[1]]<-(-2*cache$Z_list[[length(cache$Z_list)]]/(1+cache$Z_list[[length(cache$Z_list)]]^2)^2)[,t]*
        params$W_list[[length(cache$Z_list)]]%*%t(dA_list_forward[[length(cache$Z_list)-1]][t,,])
    } else
    {
      # Derivative of phi(x) is phi(x)(1-phi(x)): double derivative is (1-phi(x))phi(x)(1-2phi(x))
      da<-(1-cache$A_list[[length(cache$A_list)]])*cache$A_list[[length(cache$A_list)]]
      da<-da[,t]
      ddA_list[[1]]<-da*(1-2*cache$A_list[[length(cache$A_list)]][,t])*
        params$W_list[[length(cache$Z_list)]]%*%t(dA_list_forward[[length(cache$Z_list)-1]][t,,])
    }
  }
  #    dim(ddA_list[[1]])
  if (length(n_h)>1)
  {
    for (i in 2:length(n_h))#i<-2
    {
      ...
      {
        #        dA_listh[[i]]<-(t(params$W_list[[length(n_h)+3-i]])%*% dA_listh[[i-1]])/(1+cache$Z_list[[length(n_h)+2-i]]^2)
        ddA_list[[i]]<-(t(params$W_list[[length(n_h)+3-i]])%*% ddA_list[[i-1]])*1/(1+cache$Z_list[[length(n_h)+2-i]]^2)[,t]+
          as.vector((t(params$W_list[[length(n_h)+3-i]])%*% dA_list[[i-1]][,t])*(-2)*(cache$Z_list[[length(n_h)+2-i]]/(1+cache$Z_list[[length(n_h)+2-i]]^2)^2)[,t])*
          params$W_list[[length(n_h)+2-i]]%*%t(dA_list_forward[[length(n_h)+1-i]][t,,])
      } else
      {
        #        dA_listh[[i]]<-(t(params$W_list[[length(n_h)+3-i]])%*% dA_listh[[i-1]])*(1-cache$A_list[[length(n_h)+2-i]])*cache$A_list[[length(n_h)+2-i]]
        da<-((1-cache$A_list[[length(n_h)+2-i]])*cache$A_list[[length(n_h)+2-i]])[,t]
        ddA_list[[i]]<-(t(params$W_list[[length(n_h)+3-i]])%*% ddA_list[[i-1]])*da+
          as.vector((t(params$W_list[[length(n_h)+3-i]])%*% dA_list[[i-1]][,t])*da*(1-2*cache$A_list[[length(n_h)+2-i]])[,t])*
          params$W_list[[length(n_h)+2-i]]%*%t(dA_list_forward[[length(n_h)+1-i]][t,,])
        if (F)
        {
          # Alternative expression: consistent with paper since we exchanged ordering of dA_list[[i-1]] and params$W_list[[length(n_h)+2-i]]%*%t(dA_list_forward[[length(n_h)+1-i]][,k,])
  ...
  if (atan_not_sigmoid)
  {
    #    ddA_list[[i+1]]<-(t(params$W_list[[2]])%*% ddA_list[[i]])*dA_list_forward[[i]][,k,]*1/(1+cache$Z_list[[1]]^2)+
    #      (t(params$W_list[[2]])%*% dA_list[[i]])*(-2)*cache$Z_list[[1]]/(1+cache$Z_list[[1]]^2)^2
    ddA_list[[i+1]]<-(t(params$W_list[[2]])%*% ddA_list[[i]])*(1/(1+cache$Z_list[[1]]^2))[,t]+
      as.vector(t(params$W_list[[2]])%*% dA_list[[i]][,t])*(-2)*(cache$Z_list[[1]]/(1+cache$Z_list[[1]]^2)^2)[,t]*params$W_list[[1]]
  } else
  {
    da<-((1-cache$A_list[[1]])*cache$A_list[[1]])[,t]
    ddA_list[[i+1]]<-(t(params$W_list[[2]])%*% ddA_list[[i]])*da+
      as.vector((t(params$W_list[[2]])%*% dA_list[[i]][,t])*da*((1-2*cache$A_list[[1]][,t])))*params$W_list[[1]]
  }
...
# Input layer
  ...
  # Here derivative with respect to x_j
  #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  Hessian<-Hessian_t<-t(params$W_list[[1]])%*% ddA_list[[i+1]]
