...
weight_trade_func<-function(xyz,quantile_select,length_roll_quantile)
{
  ...
  if (F)
  {
# Old code more general i.e. arbitrary quantile based on generic apply.rolling function.
#  Problem: error if width is too large at start
    roll_quant_up<-apply.rolling(xyz,width=length_roll_quantile,FUN="quantile",p=quantile_select,partial=T)
    roll_quant_low<-apply.rolling(xyz,width=length_roll_quantile,FUN="quantile",p=1-quantile_select,partial=T)
  }
...
# Weight is one if xyz smaller than quantile
# we use lagged quantile because a current negative outlier will pull-down the quantile, too, which is bad
...
# which is already lagged by one day when compared to y_test (the latter is traded by weighting with the weight_trade series)
...
load_BTC_func<-function(in_out_sample_separator,use_scaled_data,load_data)
{
  ...
  plot(log(dat$close),col=1,on=1,main="Log-prices")  #tail(dat$close)
  ...
  x_fit<-as.ts(na.omit(diff(log(dat$close))))
  # GARCH(1,1)
...
load_spdaily_func<-function(in_out_sample_separator,use_scaled_data,load_data,start_date,diff_data,save_data_file_name)
{
  # Load Bitcoin data
  path.dat<-paste(getwd(),"/Data/",sep="")
  #path.dat<-paste(path.main,"/Exercises/Erste Woche/Data/",sep="")
...
#    x<-na.exclude(diff(log_sp500))
    plot(x)
    save(x,file=paste(path.dat,save_data_file_name,sep=""))
  } else
  {
    load(paste(path.dat,"/",save_data_file_name,sep=""))
  }
  ...
  return(list(sp=sp,x_train=x_train,x_test=x_test,y_train=y_train,y_test=y_test,train_set=train_set,test_set=test_set,target_in=target_in,explanatory_in=explanatory_in,target_out=target_out,explanatory_out=explanatory_out,data_mat=data_mat))
...
# This has nothing to do with LPD an is used only to demonstrate that the information of LPD is not already contained in original data
  if (recompute_results)
  {
    k_BTC<-1
...
generate_quantile_QPD_adjusted_performance_func<-function(QPD_array_out_sample,k,y_test,quantile_select,length_roll_quantile,path.results,recompute_results,x_test)
{
  # Use mean QPD: note that QPD has one dimension less than LPD Iit does not have the intercept)
  # Therefore we use k (this corresponds to explanatory x_test[,k])
  xyz<-apply(QPD_array_out_sample[,,k],2,mean)
  # The dates correspond to x_test since LPD is based on x_test (its the same as y_test...)
  names(xyz)<-index(x_test)
  xyz<-as.xts(xyz)
  # The dates correspond to x_test since LPD is based on x_test
  ...
  {
    weight_obj<-weight_trade_func(xyz,quantile_select,length_roll_quantile)
    save(weight_obj,file=paste(path.results,"/weight_obj_QPD_",100*quantile_select,"_",length_roll_quantile,sep=""))
  } else
  {
    load(file=paste(path.results,"/weight_obj_QPD_",100*quantile_select,"_",length_roll_quantile,sep=""))
  }
  roll_quant_up<-weight_obj$roll_quant_up
  roll_quant_low<-weight_obj$roll_quant_low
  # Long if QPD is below upper quantile: leads to short signals during up-turns (bad)
  weight_trade_up<-weight_obj$weight_trade_up
# Check
  if (F)
  {
    which(!weight_trade_up)
# The lagged quantile is used in order to ignore possible current outliers: this does not impact shift/delay, see weight_trade_func function for details
    which(xyz>lag(roll_quant_up))
  }
#  which(!weight_trade_up)
  # Long if QPD is below upper quantile and above lower quantile: short during down-turns (good) and up-turns (bad)
  weight_trade<-weight_obj$weight_trade_two_side
  # Long if QPD is above lower quantile: leads to short signals during down-turns (good)
  weight_trade_low<-weight_obj$weight_trade_low
  # Performance when being in long position outside critical time points identified by LPD
  # Note that rolling quantile is based on lagged data (LPD is based on x_test and is therefore lagged with respect to y_test).
  perf_weight_low<-cumsum(na.exclude(y_test*weight_trade_low))
  ...
  colnames(mplot_all)[1:3]<-c("buy-and-hold","perf_weight_low","perf_weight_up")
  colnames(mplot_all)[ncol(mplot_all)]<-"Lagged QPD"

  return(list(mplot_all=mplot_all,weight_trade_low=weight_trade_low,weight_trade_up=weight_trade_up,roll_quant_up=roll_quant_up,roll_quant_low=roll_quant_low))
}
...
generate_plot_sp_func<-function(use_LPD_Hessian_std,k,LPD_array_out_sample,LPD_array_in_sample,Hessian_array_out_sample,Hessian_array_in_sample,y_test,quantile_select,length_roll_quantile,colo,sharpe_periodicity)
{
  if (use_LPD_Hessian_std=="LPD")
  {
    ...
    names(xyz_out)<-index(x_test)
    xyz_out<-as.xts(xyz_out)
    index(xyz_out)<-index(x_test)
    xyz_in<-apply(LPD_array_in_sample[,,k+1],2,mean)
    names(xyz_in)<-index(x_train)
    xyz_in<-as.xts(xyz_in)
    index(xyz_in)<-index(x_train)
    # In order to be able to apply rolling-windows of sufficient length (10 years) we append in-sample and out-sample LPD's
...
# In order to be able to apply rolling-windows of sufficient length (10 years) we append in-sample and out-sample LPD's
  ...
  weight_obj<-weight_trade_func(xyz,quantile_select,length_roll_quantile)
# Rolling upper and lower quantiles
  roll_quant_up<-weight_obj$roll_quant_up
  roll_quant_low<-weight_obj$roll_quant_low
# Market exit signals: 1 (stay in market) or 0 (exit market) signals
# Used for weighting y_test i.e. weight_trade_low*y_test
#   The signals are based on x_test and are thus already lagged by one day when compared to y_tests
# Long if LPD is below upper quantile: leads to short signals during up-turns (bad)
  weight_trade_up<-weight_obj$weight_trade_up
# Long if LPD is below upper quantile and above lower quantile: short during down-turns (good) and up-turns (bad)
...
# Performances:
# Note that rolling quantile is based on lagged data (LPD is based on x_test and is therefore lagged with respect to y_test).
# Therefore no lags are necessary here!!!
  perf_weight_low<-cumsum(na.exclude(y_test*weight_trade_low))
  perf_weight_up<-cumsum(na.exclude(y_test*weight_trade_up))
...
# Use upper-quantile standarddeviation
  ...
  return(list(mplot=mplot,plot_title=plot_title,weight=weight,roll_quant_up=roll_quant_up,roll_quant_low=roll_quant_low,xyz=xyz))
}




generate_plot_sp_func_NN_prediction<-function(use_LPD_Hessian_std,k,out_mse_out,out_mse_in,Hessian_array_out_sample,Hessian_array_in_sample,y_test,quantile_select,length_roll_quantile,colo,sharpe_periodicity)
{

# NN forecasts
# Compute consensus forecast
  xyz_out<-apply(out_mse_out,1,mean)
  names(xyz_out)<-index(x_test)
  xyz_out<-as.xts(xyz_out)
  index(xyz_out)<-index(x_test)
  # Compute consensus forecast
  xyz_in<-apply(out_mse_in,1,mean)
  names(xyz_in)<-index(x_train)
  xyz_in<-as.xts(xyz_in)
  index(xyz_in)<-index(x_train)
  # In order to be able to apply rolling-windows of sufficient length (10 years) we append in-sample and out-sample LPD's
